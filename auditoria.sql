...
FOR EACH ROW INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('puntos_venta','INSERT',null,NEW.codigo,now(),current_user());
-- update
delimiter //
 ...
 VALUES ('puntos_venta','UPDATE',OLD.codigo,NEW.codigo,now(),current_user());
        ELSEIF OLD.provincia<>NEW.provincia THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('puntos_venta','UPDATE',OLD.provincia,NEW.provincia,now(),current_user());
        END IF;
   END;//
delimiter ;
-- delete
...
FOR EACH ROW INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('puntos_venta','DELETE',OLD.codigo,null,now(),current_user());
------------------ Area

-- insert
CREATE  TRIGGER insertar_auditoria_areas AFTER INSERT ON supermercado.areas 
FOR EACH ROW INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Areas','INSERT',null,NEW.id_area,now(),current_user());
 -- update
delimiter //
    ...
    FOR EACH ROW
     BEGIN
        IF OLD.id_area<>NEW.id_area THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Areas','UPDATE',OLD.id_area,NEW.id_area,now(),current_user());
        ELSEIF OLD.nombre<>NEW.nombre THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Areas','UPDATE',OLD.nombre,NEW.nombre,now(),current_user());
        END IF;
   END;//
 -- delete
CREATE  TRIGGER delete_auditoria_areas AFTER DELETE ON supermercado.areas 
FOR EACH ROW INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Areas','DELETE',OLD.id_area,null,now(),current_user());
...
FOR EACH ROW INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos','INSERT',null,NEW.id_producto,now(),current_user());
  -- update
 delimiter //
    ...
    FOR EACH ROW
     BEGIN
        IF OLD.PLU<>NEW.PLU THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.PLU,NEW.PLU,now(),current_user());
        ELSEIF OLD.EAN<>NEW.EAN THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.EAN,NEW.EAN,now(),current_user());
  ELSEIF OLD.descripcion<>NEW.descripcion THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.descripcion,NEW.descripcion,now(),current_user());
 
   ELSEIF OLD.peso<>NEW.peso THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.peso,NEW.peso,now(),current_user());
 
   ELSEIF OLD.precio<>NEW.precio THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.precio,NEW.precio,now(),current_user());
 
   ELSEIF OLD.id_area_venta<>NEW.id_area_venta THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.id_area_venta,NEW.id_area_venta,now(),current_user());
 
   ELSEIF OLD.cod_punto_venta<>NEW.cod_punto_venta THEN
            INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos',CONCAT('UPDATE ID= ',OLD.id_producto),OLD.cod_punto_venta,NEW.cod_punto_venta,now(),current_user());
        END IF;
   END;//
...
FOR EACH ROW INSERT into auditoria_supermercado.auditoria(nombre_tabla,operacion,valor_viejo, valor_nuevo, fecha_actual,usuario)
 VALUES ('Productos','DELETE',OLD.id_producto,null,now(),current_user());